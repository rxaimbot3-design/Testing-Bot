...
function matchStream(query: string): { url: string; title: string; artist: string } {
  ...
  // Score each stream by keyword matches
...
export async function getAudioStreamDetails(query: string) {
  ...
  if (cleanQuery) {
    ...
    // Best-effort play-dl for non-YouTube URLs (SoundCloud, etc)
    ...
    // For YouTube URLs or if play-dl failed, use genre-matched radio streams
    const matched = matchStream(cleanQuery);
    songUrl = matched.url;
    
    // Try to get a meaningful title from the query
    if (isYouTube) {
      ...
      // Use the matched stream title for non-URL queries
      title = matched.title;
